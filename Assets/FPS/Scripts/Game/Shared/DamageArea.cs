...
namespace Unity.FPS.Game
{
    public class DamageArea : MonoBehaviour
    {
        ...
        float rocketJumpForce = 200;

        RocketJump player;

        private void Awake()
        {
            player = GameObject.FindGameObjectWithTag("Player").GetComponent<RocketJump>();
        }
            ...
            QueryTriggerInteraction interaction, GameObject owner)
        {
            ...
            foreach (Damageable uniqueDamageable in uniqueDamagedHealths.Values)
            {
                ...
                if (uniqueDamageable.CompareTag("Player"))
                {
                    ...
                    player.Knockback((uniqueDamageable.transform.position - center), 200);
